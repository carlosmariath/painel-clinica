...
const KanbanCalendar: React.FC<KanbanCalendarProps> = ({
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (!onMove) return;
    const appt = mapped.find(m => m.id === active.id);
    if (!appt) return;
    
    try {
      // calculate new column by delta.x
      const colWidth = (document.getElementById('kanban-grid')?.clientWidth ?? 800) / 8; // rough
      const movedCols = Math.round(delta.x / colWidth);
      const newCol = Math.max(0, Math.min(6, Math.floor((appt.startDate.getTime() - weekStart.getTime()) / (1000*60*60*24)) + movedCols));
      const newDateObj = addDays(weekStart, newCol);
      const dateStr = newDateObj.toISOString().split('T')[0];

      // calculate new time by delta.y
      const movedRows = Math.round(delta.y / SLOT_HEIGHT);
      const startMinutes = appt.startDate.getHours()*60 + appt.startDate.getMinutes() + movedRows*hourStep;
      const durMinutes = (appt.endDate.getTime() - appt.startDate.getTime())/60000;
      const newStartMin = Math.max(startHour*60, Math.min(endHour*60 - durMinutes, startMinutes));
      const newEndMin = newStartMin + durMinutes;
      const toHHMM = (m:number)=>`${Math.floor(m/60).toString().padStart(2,'0')}:${(m%60).toString().padStart(2,'0')}`;
      
      onMove(appt.id, dateStr, toHHMM(newStartMin), toHHMM(newEndMin));
    } catch {
      // Silenciar erros durante a operação de arrastar para não quebrar a UI
    }
  };
